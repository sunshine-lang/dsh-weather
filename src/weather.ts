...
function clampDays(days: number | undefined, max: number): number {
  const value = days ?? 1
  return Math.min(Math.max(value, 1), max)
}

/** Resolve a place name to coordinates, or throw when nothing matches. */
export async function geocode(location: string, timeoutMs: number): Promise<GeocodeHit> {
  ...
  if (hit === undefined) throw new Error(`Location not found: ${location}`)
  return hit
}

/** Format the current weather and optional multi-day forecast as model-facing text. */
export async function getWeather(params: WeatherParams, config: WeatherConfig): Promise<string> {
  ...
  if (current === undefined) throw new Error(`No current weather returned for ${params.location}`)
